module RedirectPatch
  def redirect_to(options = {}, response_status = {})
    if options.is_a?(String)
      super(options + '?instanceId=8a5fedcfe1e1&containerPort=3000&languageName=rails5&locale=ja', response_status)
    else
  ...
  def stylesheet_link_tag(*sources)
    ...
    sources.uniq.map { |source|
      tag_options = {
        "rel" => "stylesheet",
        "media" => "screen",
        "href" => path_to_stylesheet(source, path_options) + '&instanceId=8a5fedcfe1e1&containerPort=3000&languageName=rails5&locale=ja'
      }.merge!(options)
